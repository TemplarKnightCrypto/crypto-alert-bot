# ============================================
# The Control Tower - Templar Knight Crypto - v10.3.2
# ============================================
...
# ---- Sheets rehydrate config ----
SHEETS_OPEN_URL = os.environ.get("GOOGLE_SHEETS_WEBHOOK")  # your deployed /exec URL
SHEETS_TOKEN    = os.environ.get("SHEETS_TOKEN")           # must match Apps Script Script Property
...
class IntegratedTradeTracker:
    def __init__(self, bot):
        ...
        self.entry_times = {}
        self.partial_exits = {}  # trade_id -> list of partials

    # ---------- Discord logging (entry / partial / exit) ----------

    async def log_trade_entry(self, trade_data: dict) -> bool:
        """Log new trade entry to Discord and forward to Google Sheets (if enabled)."""
        try:
            ...
            embed.add_field(
                name="🎪 Targets",
                value=f"${float(trade_data['tp1']):.2f} / ${float(trade_data['tp2']):.2f}",
                inline=True
            )
            ...
            rr_str = "∞" if risk_pct == 0 else f"1:{(reward1_pct / risk_pct):.1f}"
            ...
            self.trade_messages[trade_data['id']] = msg.id
    ...
    async def log_partial_exit(
        ...
        *,
        ...
        """Log partial exits (e.g., TP1). If silent=True, only store analytics (no Discord post)."""
        try:
    ...
    def _store_partial_exit(
        ...
        """Keep partial exits for analytics/blended PnL."""
    ...
    async def log_trade_exit(self, trade_id: str, exit_price: float, exit_reason: str, pnl_pct: float) -> bool:
        """Finalize a trade; update the original embed if we have it, else post a fresh one. Also forward to Sheets."""
        try:
            channel = self.bot.get_channel(SCROLLS_ORDER_ID)
            if not channel:
                return False
            ...
            if message_id:
                try:
                    ...
                    embed.add_field(name="💰 PnL", value=f"{float(pnl_pct):+.2f}%", inline=True)
            ...
            logger.warning("✅ Trade exit %s: %+0.2f%%", trade_id, pnl_pct)
            return True
    ...
    # ---------- Performance report (unchanged logic) ----------
    ...
    def _calculate_performance_stats_enhanced(self, trades: list, days: int) -> dict:
    ...
    # ---------- Google Sheets bridge (robust + enhanced_data) ----------
    ...
    async def _send_to_sheets(self, data: dict, action: str) -> bool:
        ...
        if action == "entry":
            ...
            logger.warning("Sheets[entry]: has_enhanced=%s enh_keys=%s", bool(enh_keys), enh_keys)
        else:
    ...
    async def export_trades_csv(self, days: int = 30):
        """Export trades as CSV file (uses BytesIO for discord.File)."""
        try:
            channel = self.bot.get_channel(SCROLLS_ORDER_ID)
            if not channel:
                return None
            ...
            fname = f"trade_export_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.csv"
            return discord.File(BytesIO(csv_content.encode()), filename=fname)
    ...
    def _parse_trade_from_message(self, message) -> dict | None:
        """Extract trade data (score, pnl, exit reason) from a Discord embed message."""
        try:
            embed = message.embeds[0]
    ...
    def _extract_trade_id(self, message) -> str:
        """Extract trade ID from the message footer."""
        try:
            if message.embeds:
...
# ---- Rehydrate open trades from Google Sheets (GET /exec?action=open) ----
async def rehydrate_active_trades():
    """Load OPEN rows from Sheets and repopulate active_trades at startup."""
    if not SHEETS_OPEN_URL or not SHEETS_TOKEN:
        logger.warning("rehydrate: missing GOOGLE_SHEETS_WEBHOOK or SHEETS_TOKEN; skipping")
        return

    url = f"{SHEETS_OPEN_URL}?action=open"
    headers = {"X-App-Secret": SHEETS_TOKEN}
    try:
        timeout = aiohttp.ClientTimeout(total=15)
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(url, headers=headers) as resp:
                text = await resp.text()
                if resp.status != 200:
                    logger.error("rehydrate: bad status %s body=%s", resp.status, text[:300])
                    return
    ...
    for r in rows:
        ...
        if not trade_id:
            continue
        side = str(r.get("direction") or "").title()  # "Long"/"Short"
        active_trades[trade_id] = {
            "id": trade_id,
...
async def on_ready():
    ...
    # Rehydrate open trades from Sheets BEFORE starting background tasks
    try:
    ...
    try:
        # Start all tasks with FIXED versions (guarded)
        _safe_start(globals().get("enhanced_camarilla_scan"),   "enhanced_camarilla_scan")
        _safe_start(globals().get("chronicle_loop"),            "chronicle_loop")
        _safe_start(globals().get("trade_100x_scan"),           "trade_100x_scan")

        # FIXED alert tasks
        _safe_start(globals().get("enhanced_camarilla_warning"), "enhanced_camarilla_warning")
        _safe_start(globals().get("smart_battleground_monitor"), "smart_battleground_monitor")
        _safe_start(globals().get("track_setup_outcomes"),       "track_setup_outcomes")

        # Core monitoring tasks
        _safe_start(globals().get("monitor_trade_exits"), "monitor_trade_exits")
        _safe_start(globals().get("memory_cleanup"),      "memory_cleanup")
