# ============================================
# Control Tower - Fixed v11.4
# Single-file deployable build
# ============================================
...
def health_root():
    return jsonify(
        ok=True,
        service="Control Tower Fixed v11.2",
...
class BotConfig:
    ...
    def from_env() -> "BotConfig":
        ...
        trail_mode_raw = os.getenv("TRAIL_MODE", "none").upper()
        if trail_mode_raw not in ("NONE", "ATR", "CHAND"):
            trail_mode_raw = "NONE"
        trail_mode = TrailMode(trail_mode_raw) if trail_mode_raw != "NONE" else TrailMode.NONE
...
class GoogleSheetsIntegration:
    ...
    async def _post(self, session: aiohttp.ClientSession, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        last_err: Dict[str, Any] = {"status": "error", "error": "unknown"}
        ...
        for attempt in range(1, 4):
            try:
                ...
                async with session.post(self.url, headers=headers, json=payload, timeout=timeout) as resp:
                    ...
                    last_err = {"status": resp.status, "body": txt}
                    log.warning(f"Sheets POST attempt {attempt} failed: {resp.status} - {txt[:200]}")
            except Exception as e:
                last_err = {"status": "error", "error": str(e)}
                log.warning(f"Sheets POST attempt {attempt} exception: {e}")
        ...
        for r in rows:
            try:
                ...
                trade = TradeData(
                    ...
                    asset=str(r.get("asset") or "ETHUSD"),
                    direction=direction,
...
class MarketDataProvider:
    KRAKEN_PAIR_MAP = {
        "ETHUSD": "ETHUSD",   # sometimes XETHZUSD is used; ETHUSD works for /OHLC on most clusters
        "BTCUSD": "XBTUSD",
        "SOLUSD": "SOLUSD"
    }
    ...
    async def fetch_ohlc(self, n: int = 500) -> pd.DataFrame:
        ...
        async with self.session.get(url, params=params) as resp:
            if resp.status != 200:
                raise RuntimeError(f"Kraken API HTTP {resp.status}")
        ...
        if not result_keys:
            raise RuntimeError("Kraken API: no result keys")

        key = result_keys[0]
        rows = data["result"][key][-n:]
        if not rows:
            raise RuntimeError("Kraken API: empty OHLC rows")
...
def confirm_breakout(c, o, h, l, vol, avg_vol, level: float, direction: TradeDirection) -> Tuple[bool, Dict[str, Any]]:
    ...
    try:
        ...
        vol_ok = (avg_vol <= 0) or (vol >= avg_vol * 1.2)
...
# Global singletons (assigned in main)
...
async def send_battle_signal(channel: discord.TextChannel, t: TradeData):
    try:
        ...
        level_val = f"{t.level_name} ({t.level_price:.2f})" if t.level_price else (t.level_name or "?")
    ...
    async def scan_loop():
        try:
            ...
            # Setup alert (not confirmed)
                ...
                fields = {
                    "Price": f"{sig['setup'].get('price', 0):.2f}",
...
def main():
    ...
    try:
        ...
        log.info("Starting Control Tower Fixed v11.2...")

        # Load configuration
        cfg = BotConfig.from_env()
        log.info(f"Configuration loaded: pair={cfg.pair}, interval={cfg.interval_min}m, trail={cfg.trail_mode.value}")
